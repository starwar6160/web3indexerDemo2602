...
async function down() {
    ...
    await (0, kysely_1.sql) `ALTER TABLE blocks DROP CONSTRAINT IF EXISTS blocks_chain_number_unique;`.execute(db);
    await (0, kysely_1.sql) `ALTER TABLE blocks DROP CONSTRAINT IF EXISTS blocks_hash_format;`.execute(db);
    await (0, kysely_1.sql) `ALTER TABLE blocks DROP CONSTRAINT IF EXISTS blocks_parent_hash_format;`.execute(db);
    await (0, kysely_1.sql) `ALTER TABLE blocks DROP CONSTRAINT IF EXISTS blocks_timestamp_not_future;`.execute(db);
    await (0, kysely_1.sql) `ALTER TABLE sync_checkpoints DROP CONSTRAINT IF EXISTS sync_checkpoints_name_unique;`.execute(db);
    await (0, kysely_1.sql) `ALTER TABLE sync_checkpoints DROP CONSTRAINT IF EXISTS sync_checkpoints_hash_format;`.execute(db);
    await (0, kysely_1.sql) `ALTER TABLE app_locks DROP CONSTRAINT IF EXISTS app_locks_name_unique;`.execute(db);
    await (0, kysely_1.sql) `ALTER TABLE app_locks DROP CONSTRAINT IF EXISTS app_locks_expires_not_past;`.execute(db);
    await (0, kysely_1.sql) `ALTER TABLE transaction_logs DROP CONSTRAINT IF EXISTS transaction_logs_tx_log_unique;`.execute(db);
    await (0, kysely_1.sql) `ALTER TABLE transaction_logs DROP CONSTRAINT IF EXISTS transaction_logs_hash_format;`.execute(db);
    await (0, kysely_1.sql) `ALTER TABLE transaction_logs DROP CONSTRAINT IF EXISTS transaction_logs_address_format;`.execute(db);
