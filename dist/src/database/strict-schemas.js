...
exports.TransactionLogSchema = exports.DbOutputSchema = exports.TransactionSchema = exports.StrictBlockSchema = void 0;
...
exports.strictValidateLog = strictValidateLog;
exports.strictValidateLogs = strictValidateLogs;
 ...
 * 用于验证从 RPC 获取的日志数据
 */
exports.TransactionLogSchema = zod_1.z.object({
    ...
    data: zod_1.z.string().startsWith('0x'),
    topics: zod_1.z.array(zod_1.z.string().startsWith('0x')).max(4, 'Maximum 4 topics allowed'),
});
/**
 ...
 * @throws ZodError 如果数据无效
 */
function strictValidateLog(data, source = 'unknown') {
    try {
        return exports.TransactionLogSchema.parse(data);
    }
    catch (error) {
        if (error instanceof zod_1.z.ZodError) {
            logger_1.default.error({
 ...
 * 只要有一个日志无效，整个批次都拒绝
 */
function strictValidateLogs(logs, source = 'batch') {
    logger_1.default.debug({ count: logs.length, source }, 'Validating log batch');
    const validatedLogs = [];
    const errors = [];
