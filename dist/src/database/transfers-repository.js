...
class TransfersRepository {
     ...
     */
    async saveWithTrx(trx, transfers) {
        ...
        await trx
            ...
            .onConflict((oc) => oc.columns(['block_number', 'log_index']).doUpdateSet({
            transaction_hash: (trx) => trx.ref('excluded.transaction_hash'),
            from_address: (trx) => trx.ref('excluded.from_address'),
            to_address: (trx) => trx.ref('excluded.to_address'),
            amount: (trx) => trx.ref('excluded.amount'),
            token_address: (trx) => trx.ref('excluded.token_address'),
        }))
            .execute();
     ...
     */
    async getByBlockNumber(blockNumber) {
     ...
     */
    async countByBlock(blockNumber) {
        const result = await this.db
            .selectFrom('transfers')
