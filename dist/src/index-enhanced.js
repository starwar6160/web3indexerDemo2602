...
const distributed_lock_1 = require("./database/distributed-lock");
const init_lock_table_1 = require("./database/init-lock-table");
...
const INSTANCE_ID = process.env.INSTANCE_ID || (0, crypto_1.randomUUID)(); // Unique instance identifier
let isRunning = true;
async function main() {
    ...
    await (0, init_lock_table_1.initLockTable)();
    const appLock = new distributed_lock_1.AppLock();
    ...
    const syncEngine = new sync_engine_1.SyncEngine({
        ...
        tokenContract: TOKEN_CONTRACT_ADDRESS, // ERC20 token contract to monitor
    });
    ...
    try {
        ...
        const lock = new distributed_lock_1.DistributedLock('block-sync');
...
process.on('unhandledRejection', (reason, promise) => {
    fatal(reason instanceof Error ? reason : new Error(String(reason)), 'unhandledRejection');
});
// 未捕获的异常
