...
class SyncEngine {
    constructor(config) {
        ...
        this.transfersRepository = new transfers_repository_1.TransfersRepository(); // P3: Transfer events
    ...
    async syncBatch(startBlock, endBlock, expectedParentHash) {
        ...
        if (batchRange > 1000n) {
            throw new Error(`Batch size ${batchRange.toString()} exceeds safe limit of 1000 blocks. ` +
        ...
        const failedBlocks = [];
        let reorgDetectedAt = null;
        let reorgParentHash = null;
            ...
            if (reorgDetectedAt !== null) {
                ...
                if (this.config.tokenContract) {
                    ...
                    if (remainingTransfers) {
                        throw new Error(`Cascade delete failed: transfers still exist at block ${reorgDetectedAt}. ` +
        ...
        await this.blockRepository.db.transaction().execute(async (trx) => {
            ...
            if (this.config.tokenContract) {
                const remainingTransfers = await trx
                    ...
                    .executeTakeFirst();
                if (remainingTransfers) {
                    throw new Error(`Cascade delete failed: transfers still exist after block ${blockNumber}. ` +
                        `Database inconsistency detected.`);
                }
            }
        });
