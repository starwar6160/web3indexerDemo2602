...
class MetricsCollector extends events_1.EventEmitter {
    constructor() {
        ...
        this.resetPeriod = 60000; // 每分钟重置一次统计数据
     ...
     * 记录数据库写入
     */
    recordDbWrite(success, latency, blockSize) {
     ...
     * 更新同步状态
     */
    updateSyncState(currentBlock, highestBlock) {
     ...
     */
    recordRpcLatency(operation, latency) {
        this.recordRpcCall(true, latency);
    }
    /**
     * Increment blocks indexed counter
     */
    incrementBlocksIndexed(count = 1) {
        // Track in sync state
