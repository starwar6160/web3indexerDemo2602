...
import { createDbConnection, getDb } from '../database/database-config';
import { sql } from 'kysely';

async function testBigIntBoundaries(): Promise<void> {
  ...
  try {
    ...
    try {
      const result = await db
        .selectFrom('blocks')
        .select(sql<bigint>`number + 1`.as('next_block'))
        .orderBy('number', 'desc')
        .limit(1)
        .executeTakeFirst();
    ...
    try {
      ...
      console.log(`  ✅ Correctly rejected unsafe reorg: ${(error as Error).message}\n`);
    }
